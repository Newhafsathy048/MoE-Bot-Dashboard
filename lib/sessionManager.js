...
const MAX_SESSIONS = Math.max(1, parseInt(process.env.MAX_SESSIONS || '100', 10) || 100);
...
async function connectSession(number) {
  ...
  sock.ev.on('connection.update', (update) => {
    ...
    if (connection === 'close') {
      ...
      if (loggedOut) {
        ...
        console.log(chalk.red(`🚪 [${number}] Logged out. Automatically clearing sessions/${number}/ to allow re-pairing...`));
        try {
  ...
  // caller (requestPairingCode) tries to use the socket.
    // Reduced wait time - let's see if a quicker request helps bypass the closure
 ...
 */
async function restartSession(rawNumber) {
  const number = sanitizeNumber(rawNumber);
  const record = sessions.get(number);
  if (!record) throw new Error('No session found for this number.');

  console.log(chalk.cyan(`🔄 [${number}] Restarting session manually...`));
  if (record.sock) {
    try {
      record.sock.terminate();
    } catch (err) {}
  }
  return connectSession(number);
}
...
async function requestCodeWithRetry(sock, number) {
  try {
    console.log(chalk.cyan(`🔑 [${number}] Requesting pairing code...`));
    return await sock.requestPairingCode(number);
  } catch (err) {
    console.error(chalk.red(`❌ [${number}] Pairing code request failed:`), err.message);
    await new Promise((resolve) => setTimeout(resolve, 3000));
    console.log(chalk.cyan(`🔄 [${number}] Retrying pairing code request...`));
