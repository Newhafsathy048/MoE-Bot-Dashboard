...
const { isCommandEnabled, areGroupCommandsEnabled } = require('./botSettings');
...
async function handleMessage(sock, msg, record) {
  ...
  if (isGroup(from) && !areGroupCommandsEnabled()) {
    console.log(`⏸️  [${record.number}] Group command blocked globally: ${command.name}`);
    return;
  }

  if (!isCommandEnabled(command.name)) {
    console.log(`⏸️  [${record.number}] Command disabled: ${command.name}`);
    await sock.sendMessage(from, { text: `⏸️ The .${command.name} command is currently disabled.` }).catch(() => {});
    return;
  }
...
function waitForRegistration(sock) {
  return new Promise((resolve) => {
    const timer = setTimeout(() => {
      ...
      console.log('⚠️  Registration wait timed out (30s) — trying pairing anyway...');
      resolve();
    }, 30000);
...
function hasSession(rawNumber) {
  const number = sanitizeNumber(rawNumber);
  const record = sessions.get(number);
  return Boolean(record && (record.state === 'open' || record.sock?.authState?.creds?.registered));
}

async function clearSession(rawNumber) {
  const number = sanitizeNumber(rawNumber);
  if (number.length < 8) throw new Error('Enter a valid WhatsApp number with country code.');
  ...
  if (record?.sock) {
    try {
      ...
      console.warn(`Could not send logout for ${number}:`, err.message);
    }
    record.sock = null;
  }
  ...
  console.log(`🧹 [${number}] Session cleared and saved credentials removed.`);
  return { number, cleared: true };
}
