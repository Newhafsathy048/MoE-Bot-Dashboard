...
const { getBotSettings, setBotSetting, setCommandEnabled } = require('./botSettings');
...
function startServer(settings) {
  ...
  app.get('/api/commands', (req, res) => {
    ...
    res.json({
      ...
      groupCommandsEnabled: settings.groupCommandsEnabled !== false
    });
  });
  ...
  app.post('/api/group-commands', (req, res) => {
    const number = req.body?.number;
    if (!number || !sessionManager.hasSession(number)) {
      return res.status(403).json({ ok: false, error: 'Pair the bot account before changing group commands.' });
    }
    const enabled = req.body?.enabled !== false;
    setBotSetting('groupCommandsEnabled', enabled);
    return res.json({ ok: true, groupCommandsEnabled: enabled });
  });
  ...
  app.post('/api/session/clear', async (req, res) => {
    try {
      const number = req.body?.number;
      if (!number || !sessionManager.hasSession(number)) {
        return res.status(403).json({ ok: false, error: 'No active paired session found for this number.' });
      }
      const result = await sessionManager.clearSession(number);
